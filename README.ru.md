...
- Отображение текущего пульса даже при единственной точке Recorder
...
а активные калории — относительно `calorie_goal` справа. Если Recorder ещё не
накопил историю пульса, карточка показывает пунктирную линию текущего значения и
заметный маркер вместо пустого графика.
