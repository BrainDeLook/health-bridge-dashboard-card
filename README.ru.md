...
- Линейный график пульса с векторами между точками, центральной пунктирной линией и маркерами измерений
- Подсказка у каждой точки BPM со временем поступления значения в Home Assistant
- Увеличенные подписи графиков и подсказки времени для шагов и активных калорий
- Нативный графический редактор Home Assistant с выбором сущностей
- Быстрый импорт сущностей для систем с несколькими профилями Health Bridge
...
В верхней части редактора находится поле **Быстрый импорт профиля**. Оно собирает
доступные профили по суффиксам сущностей Health Bridge. После выбора карточка
сразу заполняет все найденные привязки показателей и устанавливает `user_id`.
Импорт служит стартовой настройкой: затем в разделе **Сущности показателей**
можно вручную заменить любую отдельную сущность. Выбор другого профиля заменяет
текущую таблицу привязок сущностями нового профиля.
...
а активные калории — относительно `calorie_goal` справа. При наведении, фокусе
или касании дневного столбца шагов либо точки калорий показываются значение и
время поступления исходного состояния в Home Assistant. Все подписи графиков
увеличены для лучшей читаемости. График пульса использует
...
Каждое записанное значение BPM соединяется со следующим прямым вектором без
горизонтальных «полок» и без искусственного продолжения последней точки до
правого края. В центре графика постоянно находится дополнительная пунктирная
опорная линия. При единственном значении резервная линия остаётся горизонтальной
и отображается вместе с числовой меткой. При наведении на точку мышью, фокусе или
касании на телефоне появляется подсказка с BPM, датой и временем поступления
состояния в Home Assistant. Это время получения данных сервером, а не время
фактического медицинского измерения. Графики пульса и активности используют
одинаковый масштаб координат, поэтому их шкалы, подписи, точки и подсказки имеют
одинаковый видимый размер.
